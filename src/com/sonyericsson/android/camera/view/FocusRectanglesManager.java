...
public class FocusRectanglesManager {
     ...
     */
    public void changeRectangle(CapturingMode var1, FocusMode var2_3, SmileCapture var3_2) {
        this.inactivate();
        switch (.$SwitchMap$com$sonyericsson$android$camera$configuration$parameters$CapturingMode[var1.ordinal()]) {
            case 1: {
                switch (.$SwitchMap$com$sonyericsson$android$camera$configuration$parameters$FocusMode[var2_3.ordinal()]) {
                    default: {
...
lbl39: // 2 sources:
    ...
    public void setAfRectanglePosition(int n, int n2, int n3) {
        ...
        Rect[] arrrect2 = CoordinateUtil.convertPositionToSurface(FocusRectPositionConvertor.getMultiFocusRectangles(this.mCapturingMode.getCameraId()), n, n2, n3);
        this.mMultiRectangles.setPosition(arrrect2);
    }
    ...
    public void setupRectangles(Activity activity, FrameLayout frameLayout, Rect rect) {
        this.mSingleRectangles = new SingleRectangle((ViewGroup)frameLayout);
        if (this.mFocusRectanglesViewList != null) {
            SingleRectangle singleRectangle = this.mSingleRectangles;
            LayoutInflater layoutInflater = activity.getLayoutInflater();
            View[] arrview = new View[]{this.mFocusRectanglesViewList.singleAfView};
            singleRectangle.init(layoutInflater, arrview);
        } else {
            this.mSingleRectangles.init(activity.getLayoutInflater(), null);
        }
        this.mMultiRectangles = new MultiRectangles((ViewGroup)frameLayout);
        int n = com.sonyericsson.android.camera.util.capability.HardwareCapability.getCapability((int)0).MAX_MULTI_FOCUS_AREAS.get();
        this.mMultiRectangles.init(activity.getLayoutInflater(), n);
        this.mTouchRectangles = new TouchRectangle((ViewGroup)frameLayout);
        if (this.mFocusRectanglesViewList != null) {
            TouchRectangle touchRectangle = this.mTouchRectangles;
            LayoutInflater layoutInflater = activity.getLayoutInflater();
            View[] arrview = new View[]{this.mFocusRectanglesViewList.touchAfView};
            touchRectangle.init(layoutInflater, arrview);
        } else {
            this.mTouchRectangles.init(activity.getLayoutInflater(), null);
        }
        this.mFaceRectangles = new DetectedFaceRectangles((ViewGroup)frameLayout);
        if (this.mFocusRectanglesViewList != null) {
        ...
        this.mObjectRectangle = new ObjectTrackingRectangle((ViewGroup)frameLayout);
        if (this.mFocusRectanglesViewList != null) {
            ObjectTrackingRectangle objectTrackingRectangle = this.mObjectRectangle;
            LayoutInflater layoutInflater = activity.getLayoutInflater();
            View[] arrview = new View[]{this.mFocusRectanglesViewList.trackedObjectView};
            objectTrackingRectangle.init(layoutInflater, arrview);
        } else {
    ...
    extends Enum<RectangleState> {
        ...
        static {
            INACTIVE = new RectangleState();
            SELECTED = new RectangleState();
            TRANSPARENT = new RectangleState();
            DEFAULT = new RectangleState();
            RectangleState[] arrrectangleState = new RectangleState[]{INACTIVE, SELECTED, TRANSPARENT, DEFAULT};
            $VALUES = arrrectangleState;
        }
        ...
        public static RectangleState valueOf(String string) {
            return (RectangleState)Enum.valueOf((Class)RectangleState.class, (String)string);
        }

        public static RectangleState[] values() {
            return (RectangleState[])$VALUES.clone();
        }
    }
