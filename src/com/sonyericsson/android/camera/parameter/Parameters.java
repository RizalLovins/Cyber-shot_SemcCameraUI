...
implements ParameterApplicable {
    ...
    public Parameters(CapturingMode capturingMode, boolean bl, StorageController storageController) {
        ...
        this.mHolders = new EnumMap((Class)ParameterKey.class);
    ...
    public static Map<String, String> createStringMap(List<ParameterValueHolder<?>> list) {
        ...
        for (ParameterValueHolder parameterValueHolder : list) {
            ...
            String string = parameterValueHolder.createValueString();
            hashMap.put((Object)parameterKey.toString(), (Object)string);
        }
        return hashMap;
    }
    ...
    public static void parseStringMap(List<ParameterValueHolder<?>> list, Map<String, String> map) {
        for (ParameterValueHolder parameterValueHolder : list) {
            String string = (String)map.get((Object)parameterValueHolder.get().getParameterKey().toString());
    ...
    public void commit() {
        for (ParameterKey parameterKey : this.mHolders.keySet()) {
            ...
            ((ParameterValueHolder)this.mHolders.get((Object)parameterKey)).onApplied();
        }
    }
    ...
    public List<ParameterValue> getChangedValues(EnumMap<ParameterKey, ParameterValue> enumMap) {
        ...
        for (ParameterValue parameterValue : this.getParameters().values()) {
            ...
            if (enumMap.containsKey((Object)parameterKey)) {
                ...
                arrayList.add((Object)parameterValue);
                continue;
            }
            arrayList.add((Object)parameterValue);
        }
    ...
    public ParameterValue[] getOptions(ParameterKey parameterKey) {
        if (this.mHolders.containsKey((Object)parameterKey)) {
            return ((ParameterValueHolder)this.mHolders.get((Object)parameterKey)).getOptions();
        }
    ...
    public EnumMap<ParameterKey, ParameterValue> getParameters() {
        ...
        for (ParameterKey parameterKey : this.mHolders.keySet()) {
            enumMap.put((Enum)parameterKey, ((ParameterValueHolder)this.mHolders.get((Object)parameterKey)).get());
        }
        return enumMap;
    }
    ...
    protected void putHolders(List<ParameterValueHolder<?>> list) {
        ...
        while (iterator.hasNext()) {
            this.updateHolder((ParameterValueHolder)iterator.next());
        }
    }

    public void readSharedPrefs(SharedPreferencesUtil sharedPreferencesUtil) {
        Map<String, String> map = sharedPreferencesUtil.getStringMap(SharedPreferencesUtil.createPrefix(ParameterCategory.CAPTURING_MODE, this.capturingMode, ""));
        Parameters.parseStringMap(this.mCapturingModeParams.values(), map);
    }

    public void reset() {
        for (ParameterKey parameterKey : this.mHolders.keySet()) {
            ((ParameterValueHolder)this.mHolders.get((Object)parameterKey)).reset();
            ((ParameterValueHolder)this.mHolders.get((Object)parameterKey)).setDefaultValue().apply(this);
        }
        this.commit();
    }
    ...
    protected void updateHolder(ParameterValueHolder<?> parameterValueHolder) {
        ...
        if (ParameterSelectability.getSelectability(parameterValueHolder.getOptions().length) != ParameterSelectability.INVALID) {
            this.mHolders.put((Object)parameterKey, parameterValueHolder);
            return;
        } else {
    ...
    public void updateLocalResolution(Hdr hdr, ParameterValueHolder<Resolution> parameterValueHolder) {
        ...
        parameterValueHolder.setOptions((ParameterValue[])Resolution.getHdrDependentOptions(parameterValueHolder2.getOptions(), bl, n));
